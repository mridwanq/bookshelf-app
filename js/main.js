...
function checkEmptyList(
  items,
  bookListCompletedElement,
  bookListNotCompletedElement
) {
  const completed404 = bookListCompletedElement.querySelector('.card-404');
  const notCompleted404 =
    bookListNotCompletedElement.querySelector('.card-404');
...
function render() {
  ...
  const bookListNotCompletedElement =
    document.querySelector('#is-not-completed');
  ...
  items.forEach((book) => {
    ...
    bookElement.innerHTML = `
            ...
            <div>
                <button class="btn btn-edit" onclick="updateBook(${
                  book.id
                })">Edit</button>
                <button class="btn btn-delete" onclick="removeBook(${
                  book.id
                })">Delete</button>
                <button class="btn btn-move" onclick="moveBook(${
                  book.id
                })">Set to ${book.isComplete ? 'not' : ''} finished</button>
            </div>
        `;
